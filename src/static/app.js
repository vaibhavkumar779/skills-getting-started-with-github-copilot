document.addEventListener("DOMContentLoaded", () => {
          ...
          ? `<p><strong>Participants:</strong></p>
             <ul class="participants-list">
               ${details.participants
                 .map(
                   (email) =>
                     `<li><span>${email}</span><button type="button" class="participant-remove-button" data-activity="${encodeURIComponent(
                       name
                     )}" data-email="${encodeURIComponent(email)}" aria-label="Remove ${email} from ${name}">✕</button></li>`
                 )
                 .join("")}
             </ul>`
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      ...
      messageDiv.textContent = "Unable to unregister participant. Please try again.";
      messageDiv.className = "message error";
